...
**Resilience: 529 → Ollama fallback (2026-05-22).** `CircuitBreaker.trip()` opens the breaker immediately on HTTP 529 (Overloaded) and 503 — `_retry()`, `AnthropicClient.stream()`, and `AnthropicClient.stream_with_tools()` all detect those status codes and call `cb.trip(msg)` instead of `record_failure(msg)`. The streaming methods don't go through `_retry`, so the detection lives in each path. After one 529, `SYMBION._active()` skips Anthropic and returns the next provider in `cfg.fallback_chain`; the breaker auto-resets after `_reset_after` (60s by default) so the next turn after the window tries Anthropic again. `main()` always appends `"ollama"` to `fallback_chain` when not already present (and when Ollama isn't the primary), so a 529 burst that takes out OpenAI too still has a local terminus — `mistral` + `llama3.2` from `scripts/install-ollama.ps1` cover it. Known limit: the turn that triggers the 529 still returns empty; only subsequent turns get the fallback. In-turn fallback (retry on Ollama mid-respond) would be a bigger refactor.
