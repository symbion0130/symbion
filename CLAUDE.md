...
**Agent loop boundaries.** `agent_loop_enabled` in SymbionConfig (default True). Active only when (a) tools_enabled, (b) cfg.agent_loop_enabled, (c) responder client has `supports_tools = True` (currently AnthropicClient only). Tool schemas in `TOOL_SCHEMAS` must stay in sync with `SymbionTools._ALLOWED_TOOLS` and `_validate_args`. Iteration cap is `agent_loop_max_iterations` (default 30). The loop has no token-aware backpressure — a tool that returns plausible-but-wrong content can burn the whole cap before synthesizing. `stuck_tool_name` catches the simpler failure mode (same tool name fired ≥2 times) and forces synthesis on the next iteration.
...
**Self-source pre-fetch in agent-loop mode (2026-05-23, two-tier split 2026-05-24).** `TurnPipeline.prefetch_self_source()` has TWO cost tiers gated on two separate regexes — the original unified version caused 429s on "self review" queries by dumping 140K tokens of source on every match, then the agent loop's follow-on tool reads multiplied that context across N iterations and tripped Anthropic's 450K input-tokens/min org limit.

- **`_SELF_SOURCE_RE` (narrow, source-wanting):** queries that explicitly want the code — "walk me through respond()", "read symbion_v14.py", "your codebase", "your pipeline". Pre-fetch injects **manifest + full symbion_v14.py source** (~140K tokens). The cost is justified because the source IS what the user asked for; without it the model confabulates class names from training data (iid=193 fabrication).
- **`_SELF_REVIEW_RE` (broader, opinion-wanting):** queries asking for the model's evaluation — "self review", "self-audit", "audit yourself", "critique yourself", "assess yourself honestly". Pre-fetch injects **manifest ONLY** (~1KB). The model uses the agent loop's tool calls to pull source if it actually needs it. Trades always-on grounding for budget safety.

Both regexes resolve in `prefetch_self_source()` via `want_source = _SELF_SOURCE_RE.search(text); want_review = _SELF_REVIEW_RE.search(text)`. Source-wanting wins when both match. The manifest is `list_dir(".") + list_dir("tests") + list_dir("tests/integration")` wrapped in `[Project structure -- ground-truth listing, do not assert subsystem absence without checking this. Use read_file via the agent loop to pull source on demand.]`. Closes the absence-claim failure mode (Symbion claiming "zero integration test coverage" without listing tests/) while keeping the budget under control.

**Test coverage for the split:** `tests/test_tools.py::TestSelfSourceRegex` + `TestSelfReviewRegex` lock in which queries hit which regex (and crucially, that "self review" hits ONLY the manifest-only path). `tests/integration/test_respond_assembly.py::test_self_source_prefetch_injects_source_and_manifest` asserts source-wanting queries get source + manifest; `test_self_review_prefetch_injects_manifest_only` asserts review queries get manifest WITHOUT source. If a regex consolidation regresses, both tests fail loudly.
...
**Import-time defensive checks (2026-05-24 followup).** Imported techniques surface in future system prompts via the techniques retrieval block, so whoever can write the shared file controls a prompt-injection vector. Four checks in `_parse_shared_learnings_file`:
- **File-size cap** (`_SHARED_MAX_FILE_BYTES=10MB`): refuse to parse anything larger. Returns `[]` and logs; defends against a single file stuffing the prompt at import time.
- **Per-field length caps** (`_SHARED_MAX_MOVE=500`, `_SHARED_MAX_QUERY=1000`, `_SHARED_MAX_EVIDENCE=1500`, `_SHARED_MAX_USER=32`): truncate to in-process limits. Mirrors the same caps `SymbionTools._validate_args` applies to `promote_technique` from the model side.
- **Hash verification**: if the block header carries `hash:XXXX`, recompute `sha256(user+query+move)[:12]` from the parsed fields and SKIP the entry on mismatch (with a warning). Catches both file corruption and post-export tampering. Missing-hash entries get the recomputed hash and are kept (legacy compatibility).
- **Prompt-injection marker scrub** (`_SHARED_INJECTION_MARKERS`): reject entries whose fields contain `[TOOL_DATA`, `[/TOOL_DATA]`, `[SYMBION_REVISE]`, `[THINKING_START]`, or `[THINKING_END]`. A legitimate move/query/evidence never contains these as content; presence is the obvious vector for faking tool output or thinking blocks downstream.

Tests: `tests/test_tools.py::TestSharedLearningsImportIntegrity` (10 cases) covers normal round-trip, size cap, hash mismatch skip, missing-hash recompute, 5 injection markers across the three text fields, and oversized-field truncation. Lock in the contracts so future "make the import more permissive" changes can't quietly re-open the vector.
...
- **2026-05-24 (followup, OfflineJudgeStub null-object refactor)** — surfaced by Symbion's third self-review of the day (verified 4/7 right, 2/7 partially right, 1/7 wrong). The 22 `isinstance(client, OfflineJudgeStub)` checks scattered across SYMBION (background tasks, pre-gen, self-eval, contradictions, gap check, sycophancy, technique extraction, terminal display, etc.) were class-coupled dispatch -- adding a future degraded client type would require touching every site. Replaced with `getattr(client, 'is_degraded', False)` everywhere; added `is_degraded: bool = False` to `BaseClient` (default) with `is_degraded = True` on `OfflineJudgeStub`. Behavior preserved exactly -- every early-exit path is unchanged, the dispatch is just attribute-based instead of type-based now. Three additional bumps in the same session: `agent_loop_max_iterations` CLAUDE.md doc fix (was stale at 8, actual is 30); `read_file` defaults bumped 2M -> 5M with validator cap 5M -> 10M for comfortable headroom as symbion_v14.py grows past 500 KB; new `_PREGEN_RISK_RE` false-positive regression noted (catches "a minor bug" and "manipulate variables academically" -- known issue, broader fix would need a semantic classifier). 107/107 unit + 8/8 stubbed integration green.
- **2026-05-24 (followup, shared_learnings import integrity)** — surfaced by Symbion's self-review (the 1/6 claims that audited TRUE: no integrity check on the imported markdown, content lands in future system prompts via the techniques retrieval block). Four defensive checks added to `SymbionMemory._parse_shared_learnings_file`: 10MB file-size cap, per-field length caps mirroring `_validate_args` (move<=500, query<=1000, evidence<=1500, user<=32), hash verification (skip on stored-vs-recomputed mismatch -- catches corruption + tampering), and prompt-injection marker scrub (reject entries whose fields contain `[TOOL_DATA`, `[/TOOL_DATA]`, `[SYMBION_REVISE]`, `[THINKING_START/END]` -- the obvious vector for faking system markers downstream). 10 new unit tests in `TestSharedLearningsImportIntegrity`. 107/107 unit green. **For the current threat model** (personal OneDrive, single trusted user) the realistic concern was file corruption from a partial OneDrive write -- hash recheck catches that loud. For any future multi-user / shared-path scenario the same checks defend against the adversarial case.
- **2026-05-24 (followup, cost-tier split for self-review pre-fetch)** — Symbion hit Anthropic 429 (450K input-tokens/min org limit) during a "self review" turn: the 2026-05-24 manifest extension was injecting 140K tokens of `symbion_v14.py` source on every `_SELF_SOURCE_RE` match, then the agent loop multiplied that context across 9 follow-on file reads, blowing past the per-minute budget. Fix: split the regex into `_SELF_SOURCE_RE` (narrow, source-wanting -> manifest + source) and `_SELF_REVIEW_RE` (broader, opinion-wanting -> manifest ONLY). Model uses tools to pull source on demand when it actually needs it. `TurnPipeline.prefetch_self_source()` checks both regexes and picks the cheap tier when only review matches. Unit tests `TestSelfSourceRegex` + `TestSelfReviewRegex` lock in which queries hit which tier; integration tests `test_self_source_prefetch_injects_source_and_manifest` + `test_self_review_prefetch_injects_manifest_only` lock in the live behavior of each tier. 97/97 unit + 6/6 assembly integration green. **Lesson:** any pre-fetch that injects more than ~1KB into the system prompt needs a triggering-condition narrow enough that the cost is paid only when actually justified -- the agent-loop multiplier turns moderate per-call costs into rate-limit thrash fast.
