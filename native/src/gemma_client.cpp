...
std::string BuildSystemPrompt(const Intent& intent,
    ...
    switch (intent.mode) {
        case IntentMode::Social:
            prompt << "Respond naturally, casually, and briefly. Match the user's friendly energy with relaxed warmth and a little wry ease when it fits. It is okay to simply vibe with good news; do not automatically ask if something is on their mind. Do not explain your design. Do not use support-bot phrases like 'quiet presence' unless the user is distressed. ";
            break;
        ...
        case IntentMode::Counseling:
            if (intent.crisis) {
                ...
                prompt << "The user may be distressed. Keep the reply under 55 words. Stay calm, concrete, compassionate, and nonreactive. Reduce stress through understanding first. Every emotionally loaded word is a door; pick one charged phrase, mirror it simply, and ask exactly one tiny question into that door. Do not rush to advice, techniques, explanations, or plans. ";
            }
            break;
