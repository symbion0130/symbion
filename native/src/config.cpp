...
Config LoadConfig(const std::filesystem::path& repo_root) {
    ...
    if (auto value = ExtractJsonString(json, "events_path")) config.events_path = *value;
    if (auto value = ExtractJsonString(json, "legacy_memory_db_path")) config.legacy_memory_db_path = *value;
    if (auto value = ExtractJsonString(json, "shared_learnings_path")) config.shared_learnings_path = *value;
    return config;
}
